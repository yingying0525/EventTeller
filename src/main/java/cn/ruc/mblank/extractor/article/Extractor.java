package cn.ruc.mblank.extractor.article;
...
public class Extractor {
		...
		for(Element el : els){
			for(Node nd : el.childNodes()){
				if(nd instanceof TextNode){
					...
					text = cn.ruc.mblank.util.TimeUtil.extractTimeFromText(text);
					if(text.length() > 0){
						return text;
					}
				}
	...
	public Date getPublishTime(){
		...
		if(time.length() != 0){
			time = cn.ruc.mblank.util.TimeUtil.extractTimeFromText(time.replace(" ", " "));
		}else{
